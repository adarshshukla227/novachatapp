...
const GroupInfoPanel = ({
  ...
  useEffect(() => {
    const fetchMembers = async () => {
      try {
        const res = await fetch(`/api/chat/${chatId}/members`, { credentials: "include" });
  ...
  useEffect(() => {
    const fetchGroupInfo = async () => {
      try {
        ...
        if (chat?.groupDescription) { setDescription(chat.groupDescription); setTempDesc(chat.groupDescription); }
        if (chat?.groupAvatar) setGroupAvatar(chat.groupAvatar);
        if (chat?.groupName) { setCurrentGroupName(chat.groupName); setTempName(chat.groupName); }
      } catch (err) { console.error(err); }
    };
    fetchGroupInfo();
  }, [chatId]);

  const saveGroupInfo = async (updates: { groupName?: string; groupDescription?: string; groupAvatar?: string }) => {
    setSaving(true);
    try {
    ...
    } catch { toast.error("Failed to update group info"); }
  ...
  const handleAddSelectedMembers = async () => {
    ...
    try {
      ...
      const data = await res.json();
      setMembers(data.members || []);
      toast.success(selectedUsers.length === 1 ? "Member added successfully" : `${selectedUsers.length} members added successfully`);
  ...
  const handleClearChat = async () => {
    setClearing(true);
    try {
      const res = await fetch(`/api/chat/${chatId}/clear`, { method: "DELETE", credentials: "include" });
  ...
  const handleLeave = async () => {
    setLeaving(true);
    try {
      const res = await fetch(`/api/chat/${chatId}/leave`, { method: "POST", credentials: "include" });
  ...
  const filteredAddUsers = allUsers.filter((u) => u.name.toLowerCase().includes(addSearchQuery.toLowerCase()));
  ...
  return (
    <div className="fixed inset-0 z-[200] flex justify-end">
      ...
      {/*
        KEY FIX:
        - w-full on small screens, max-w-xs caps it on larger screens
        - h-screen + overflow-y-auto on THIS div = entire panel scrolls as one page
        - No nested scroll containers
      */}
      <div className="relative w-full max-w-xs sm:max-w-sm h-screen bg-card border-l border-border shadow-2xl flex flex-col z-10 overflow-y-auto">
        ...
        <div className="flex flex-col items-center px-4 py-5 border-b border-border gap-3">
            ...
            {editingDesc ? (
              <div className="flex flex-col gap-2 text-left">
                ...
                <div className="flex gap-2">
                  <button onClick={() => { setEditingDesc(false); setTempDesc(description); }}
                    className="flex-1 py-1.5 text-xs rounded-lg border border-border hover:bg-muted transition">Cancel</button>
              ...
              <button onClick={() => isAdmin && setEditingDesc(true)}
                ...
                {description
                  ...
                  : <span className="text-primary font-medium">{isAdmin ? "+ Add group description" : "No description"}</span>}
              </button>
            )}
          </div>
        </div>
        ...
        {/* ── Action Buttons ── */}
        <div className={`grid gap-2 px-3 py-3 border-b border-border ${isAdmin ? "grid-cols-3" : "grid-cols-2"}`}>
          {isAdmin && (
            <button onClick={handleShowAddMember}
          ...
          <button onClick={() => setShowMemberSearch((p) => !p)}
        ...
        {showAddMember && (
          <div className="px-3 py-3 border-b border-border">
            <div className="flex items-center gap-2 mb-3">
              ...
              <button onClick={() => { setShowAddMember(false); setSelectedUsers([]); setAddSearchQuery(""); }}
                ...
                {selectedUsers.map((id) => {
                  ...
                  return (
                    <div key={id} className="flex items-center gap-1 bg-primary/10 text-primary text-xs px-2 py-1 rounded-full">
                      <span>{u.name}</span>
                      <button onClick={() => toggleSelectUser(id)} className="hover:opacity-70"><X size={10} /></button>
                    </div>
                  );
                })}
              </div>
            ...
            <input value={addSearchQuery} onChange={(e) => setAddSearchQuery(e.target.value)}
            ...
            <div className="max-h-40 overflow-y-auto space-y-1 mb-3">
              {filteredAddUsers.length === 0
                ...
                : filteredAddUsers.map((u) => {
                  ...
                  return (
                    <div key={u._id} onClick={() => toggleSelectUser(u._id)}
            ...
            {selectedUsers.length > 0 && (
              <button onClick={handleAddSelectedMembers} disabled={addingMember}
            ...
            filteredMembers.map((member) => (
              <div key={member._id} className="flex items-center gap-3 px-4 py-2.5 hover:bg-sidebar-accent transition-colors">
                <div className="relative flex-shrink-0">
                  <AvatarWithBadge name={member.name} src={member.avatar} isGroup={false} isOnline={false} />
                  {member.isAdmin && (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {member.isAdmin && <p className="text-xs text-yellow-600 dark:text-yellow-400 font-medium">Group Admin</p>}
                </div>
              </div>
            ))
          )}
        ...
        {showClearConfirm && (
          <div className="px-4 py-4 border-t border-border">
            <p className="text-sm text-muted-foreground text-center mb-3">Clear all messages in this chat?</p>
            <div className="flex gap-2">
        ...
        {/* ── Leave Group ── always visible at bottom, part of scroll flow ── */}
        <div className="p-4 border-t border-border mt-4">
          {!showConfirm ? (
            ...
            <div className="text-center space-y-3">
              <p className="text-sm text-muted-foreground">"{currentGroupName}" Are you sure you want to leave?</p>
              <div className="flex gap-2">
                <button onClick={() => setShowConfirm(false)}
                  className="flex-1 py-2 rounded-lg border border-border text-sm font-medium hover:bg-sidebar-accent transition">Cancel</button>
